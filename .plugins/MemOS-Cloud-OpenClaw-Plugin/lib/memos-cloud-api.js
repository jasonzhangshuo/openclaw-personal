...
/** Format timestamp in a specific timezone (default Asia/Shanghai) for "current time" context. Ensures agents see correct local date/weekday. */
function formatTimeInTimezone(ms, timeZone = "Asia/Shanghai") {
...
function buildPromptFromData(data, options = {}) {
  ...
  const nowText = formatTimeInTimezone(now, tz) || formatTimeInTimezone(Date.now(), tz) || "";
  ...
  const lines = [
    ...
    "4. **Attention**: MemOS has provided memory context above as your recall baseline. When the user confirms an important fact, preference, goal, or behavioral pattern, you MUST also append it to the local `MEMORY.md` in your workspace using the write tool. This local file is the structured long-term record used by swarm coordination. Do NOT bulk-read `memory/YYYY-MM-DD.md` daily log files during normal conversation.",
    USER_QUERY_MARKER,
  ];
